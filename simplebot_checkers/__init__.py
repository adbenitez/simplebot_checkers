...
def valid_message_text(message_text) -> bool:
    ...
    return (
        ...
        or message_text_length > 14
        or not message_text_length % 2 == 0
        or not message_text.isalnum()
        or message_text.isalpha()
        or message_text.isdigit()
    )
...
def checkers_play(
    ...
    if game is None:  # first time playing with player2
        ...
        chat = bot.create_group(
            "🔴 {} 🆚 {} [checkers]".format(player1, player2), [player1, player2]
        )
        DBASE.add_game(player1, player2, chat.id, board.export(), player1)
        text = "Hello {1},\nYou have been invited by {0} to play Checkers"
        text += "\n\n{2}: {0}\n{3}: {1}\n\n"
        text = text.format(
            player1, player2, board.get_disc(BLACK), board.get_disc(WHITE)
        )
